...
//----------------------- PARTE PRACTICA -----------------------------
  ...
  let inventarioSinTrampa = inventario.filter(powerup => !powerup.includes('trampa'));
  inventarioSinTrampa.push('super estrella');
  let inventarioEnMayusculas = [];
 inventarioSinTrampa.forEach(nombre => {
  inventarioEnMayusculas.push(nombre.toUpperCase());
  });
  let primerItem = inventarioEnMayusculas[0];
  let secondItem =  inventarioEnMayusculas[1];
